...
export function CollectionDetails({ collection }: CollectionDetailsProps) {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading, isError } = useCollection(collection.slug, filters);
    ...
    <>
      <div className="bg-background pt-12 pb-8 px-4 border-b border-border">
        <div className="container mx-auto">
          <h1 className="text-5xl md:text-7xl font-black italic uppercase tracking-tighter mb-4 text-foreground">{collection.title}</h1>
          <p className="text-muted-foreground max-w-2xl text-[10px] font-black uppercase tracking-[0.2em]">
            {collection.description || "Premium fine art prints and football posters designed to elevate your environment."}
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between py-6 mb-8 border-y border-border">
          <button 
            className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest border border-border px-6 py-3 hover:bg-primary hover:text-primary-foreground transition-colors"
          ...
          <div className="flex items-center gap-4">
            ...
            <select 
              ...
              className="bg-transparent border-none text-[10px] font-bold uppercase tracking-widest cursor-pointer focus:ring-0 outline-none text-foreground"
            >
              <option value="" className="bg-background">Featured</option>
                      ...
                      <label key={f.value} className="flex items-center gap-3 cursor-pointer group">
                        <div className="relative flex items-center justify-center">
                          <input 
                            ...
                            className="peer appearance-none w-4 h-4 border border-border rounded-none checked:bg-primary checked:border-primary transition-all cursor-pointer"
                          />
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest group-hover:text-primary transition-colors">{f.value}</span>
                        <span className="text-[10px] font-bold text-muted-foreground/60 ml-auto">{f.count}</span>
                      </label>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    {facets?.color?.map((f: any) => (
                      <label key={f.value} className="flex items-center gap-3 cursor-pointer group">
                        ...
                        <div className="w-2 h-2 border border-border shrink-0" style={{ backgroundColor: f.value }} />
                        <span className="text-[10px] font-bold uppercase tracking-widest group-hover:text-primary transition-colors">{f.value}</span>
                        <span className="text-[10px] font-bold text-muted-foreground/60 ml-auto">{f.count}</span>
                      </label>
                    ))}
                ...
                {hasNextPage && (
                  <div className="mt-16 text-center">
                    <button
                      ...
                      className="border border-border px-12 py-4 text-[10px] font-bold uppercase tracking-widest hover:bg-primary hover:text-primary-foreground transition-all"
                    >
